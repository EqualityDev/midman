...
from utils.autoposter_settings import (
    ...
    toggle_autopost_task,
    delete_autopost_task,
...
class AutoPosterCog(commands.Cog):
    ...
    async def autopost_loop(self):
        ...
        for task in tasks:
            ...
            new_counter = task["loop_counter"] + LOOP_INTERVAL
            threshold = task["interval_minutes"] * 60

            if new_counter >= threshold:
                success = await self._post_message(channel, task["message"], task["user_token"])
